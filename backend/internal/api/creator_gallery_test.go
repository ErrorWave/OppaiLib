...
func TestCharacterReferenceAndGameGalleryUploads(t *testing.T) {
	...
	characterRec := httptest.NewRecorder()
	h.ServeHTTP(characterRec, multipartRequest(t, http.MethodPost, "/api/imagegen/characters", token,
		map[string]string{"name": "Alice"}, "alice.png", pngData))
	if characterRec.Code != http.StatusCreated {
		t.Fatalf("create character: %d %s", characterRec.Code, characterRec.Body.String())
	}
	var character characterView
	_ = json.Unmarshal(characterRec.Body.Bytes(), &character)
	if character.ID == 0 || character.Name != "Alice" || character.Tags == nil {
